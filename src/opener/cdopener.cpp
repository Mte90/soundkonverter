    ...
    cddbFound( false )
{
    ...
    lAlbumCover->setPixmap( QPixmap( QStringLiteral("soundkonverter/images/nocover.png") ) );
...
//     lOverlayLabel = new QLabel( i18n("Please wait, trying to read audio CD ..."), cdOpenerOverlayWidget );
    ...
    else
    {
        ...
        {
            QStringList list;
            for(const QString& desc : devices.values())
            {
                list.append( desc );
            }
            bool ok = false;
    ...
    if( !success )
    {
        QMessageBox::information(this, tr("Error"), tr("success = false, couldn't open audio device.\nPlease report this bug."));
        noCdFound = true;
        return;
    }
    ...
    restoreGeometry( settings.value( "CDOpener/geometry" ).toByteArray() );
}

CDOpener::~CDOpener()
{
    QSettings settings("soundkonverterrc", QSettings::IniFormat);
    settings.setValue( "CDOpener/geometry", saveGeometry() );
...
//     const int status = wm_cd_init( device.toLatin1().data(), "", "", "", &wmHandle );
//
...
void CDOpener::proceedClicked()
{
    ...
    if( trackCount == 0 )
    {
        QMessageBox::critical( this, tr("Error"), i18n("Please select at least one track in order to proceed.") );
        return;
    }
...
{
    QString filename = QFileDialog::getSaveFileName( this, tr("Save Cuesheet"), QDir::homePath() + "/track.cue", tr("Cuesheets (*.cue)") );
    if( filename.isEmpty() )
        return;
