    ...
    config( _config )
{
    ...
    cGenre = new QComboBox( tagsWidget );
    cGenre->setEditable( true );
    cGenre->addItems( tagEngine->genreList );
    cGenre->clearEditText();
    QCompleter *cGenreCompletion = cGenre->completer();
    if( cGenreCompletion )
    {
        cGenreCompletion->setCaseSensitivity( Qt::CaseInsensitive );
    }
...
void OptionsEditor::itemsSelected( QList<FileListItem*> items )
{
    ...
    if( selectedItems.count() == 1 )
    {
        ...
        else
        {
            ...
            for(CoverData* cover : item->tags->covers)
            {
                QPixmap pixmap;
    ...
    else // selectedItems.count() > 1
    {
        ...
        for(const FileListItem *item : selectedItems)
        {
...
void OptionsEditor::editGenreClicked()
{
    ...
    if( selectedItems.count() > 0 && selectedItems.first() && selectedItems.first()->tags )
    {
        cGenre->setCurrentText( selectedItems.first()->tags->genre );
    }
}
